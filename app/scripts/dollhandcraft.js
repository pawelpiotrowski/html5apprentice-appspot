	...
	function() {
		return {
			...
			link: function(scope, iElement, iAttr) {
				...
				var windowSize = {};
				var init = {
					start: function(id, windowSize) {
						this.defaultProperties();
						this.canvas(id);
						this.canvSize = windowSize;
						box2d.create.world();
					...
					surroundings: {
						rightWall: function() {
							add.box({
								x: (windowSize.w / 30) - 1,        // 760 / 30 + 1.1
								y: (windowSize.h / 30) / 2,        // 380px / 30 / 2
								height: windowSize.h / 30,   // 380px / 30
								width: 1,
								isStatic: true
							});
						},
				...
				iElement.attr('width', window.innerWidth).attr('height', window.innerHeight);
